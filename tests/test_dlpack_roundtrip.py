...
CPU MRs; padded layouts additionally prove byte-correct strides and genuine
zero-copy sharing (mutations travel both ways with no transfer in between).
"""
...
_MR_TYPES = (BytearrayMemoryResource, MallocMemoryResource)
_MR_IDS = ("bytearray", "malloc")
...
def test_numpy_round_trip_matches_dtype_shape_and_values(
    mr_type: type[BytearrayMemoryResource] | type[MallocMemoryResource],
    case: tuple[tuple[int, ...], str, LayoutPolicy],
) -> None:
    shape, dtype_name, policy = case
    np_dtype = np.dtype(dtype_name)
    t = empty(shape, dtype_name, layout=policy, mr=mr_type())
...
@pytest.mark.parametrize("mr_type", _MR_TYPES, ids=_MR_IDS)
def test_padded_layout_exports_byte_correct_strides(
    mr_type: type[BytearrayMemoryResource] | type[MallocMemoryResource],
) -> None:
    policy = Aligned(RowMajor(), unit_stride_alignment=64, base_alignment=64)
    t = empty((3, 5), "float32", layout=policy, mr=mr_type())
...
@pytest.mark.parametrize("mr_type", _MR_TYPES, ids=_MR_IDS)
def test_padded_layout_mutations_round_trip_zero_copy(
    mr_type: type[BytearrayMemoryResource] | type[MallocMemoryResource],
) -> None:
    policy = Aligned(RowMajor(), unit_stride_alignment=64, base_alignment=64)
